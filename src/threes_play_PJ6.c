    ...
    for(int i=s;i<n;++i){
        ...
        if(k>1){
            compose2(n,kk,k-1,i+1,cb);
             ...
             bitboard mb = 0;
             unsigned tb = TILEBAG_STATUS_FULL;
            ...
            for(int j=0;j<kk;++j){
                ...
                tilebag_status_transfer_ptr(&tb, init_bag[bindex[j]]);
            }
            ...
            for(int h=1;h<=3;++h){
                ...
                unsigned mask = 7 << (h*3-3);
                if (tb && mask){
                    bool m;
                    depth = 3;
                    float temp = evil_expecti_slide(mb,tilebag_status_transfer(tb,h),h,&m,1,10,FLT_MIN,FLT_MAX);
                //float temp = evil_expecti_slide(b,)
                //printf("%f\n",temp);
                    if(temp < min_eval){
                        min_eval = temp;
                        min_hint = h;
...
int main(int argc, char const *argv[])
{
    ...
    for(i=1;i<argc;++i){
        if (strstr(argv[i],"--weight")) {
            ...
            if ( depth_arg[0] != '\0') {
                SEARCH_DEPTH_P = SEARCH_DEPTH = atoi(depth_arg);
            }
        ...
        switch(buf[0]){
            ...
            //For Command
                ...
                if(strcmp(cmd,"login") == 0){
                    ...
                    memset(cmd, '\0', sizeof(cmd));
                    strncpy(cmd,buf+i,j-i);
                    SEARCH_DEPTH = SEARCH_DEPTH_P = atoi(cmd);
                    printf("Search depth become e = %d p = %d\n",
				    SEARCH_DEPTH, SEARCH_DEPTH_P);
		} else if(strcmp(cmd,"depth_play") == 0){
		    i=j;while(buf[i]==' ') { if(buf[i]=='\0') break; i++; }
                    j=i;while(buf[j]!=' ' && buf[j]!='\0')j++;
                    
                    memset(cmd, '\0', sizeof(cmd));
                    strncpy(cmd,buf+i,j-i);
                    SEARCH_DEPTH_P  = atoi(cmd);
		
		    printf("Search depth become e = %d p = %d\n",
		        	    SEARCH_DEPTH, SEARCH_DEPTH_P);
		} else if(strcmp(cmd,"depth_evil") == 0){
                    i=j;while(buf[i]==' ') { if(buf[i]=='\0') break; i++; } 
                    j=i;while(buf[j]!=' ' && buf[j]!='\0')j++;
                    
                    memset(cmd, '\0', sizeof(cmd));
                    strncpy(cmd,buf+i,j-i);
                    SEARCH_DEPTH  = atoi(cmd);
            ...
            // For Comment
                ...
                if(strcmp(cmd,"?") == 0){
                    ...
                    if(myRole == Evil){
                        ...
                        op = play_expecti_next(&bb ,&rw , tilebag_status_transfer(tilebag_status , evil_hint ) , evil_hint ,bonus_tile_count,generated_tile_count);
                        if(op==-1){
                            //Null Move
                            printf("#%s ??\n",currentGameID);
                            break;
                    ...
                    if(myRole == Evil){
                        ...
                        sscanf(cmd,"%1x%1x+%1x",&evil_pos,&evil_tile,&evil_hint);

                        bitboard_set(&bb, evil_pos, evil_tile);
                        stat_action_finished(0,evil_pos,evil_tile,score_deltas[evil_tile]);

                        tilebag_status_transfer_ptr(&tilebag_status , evil_tile );

                        bonus_tile_count += (evil_tile >= 4) ? 1 : 0;
                        generated_tile_count++;
                    }
                }
