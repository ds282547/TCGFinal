        ...
        //for(unsigned bonus=4;bonus <= largest-3;bonus++){
            for(unsigned nhint = 1 ; nhint <= 3 ; ++nhint){
                if (temp_tilebag & 7) {
                    ...
                    for(unsigned i=0;i<emptyslots_length;++i){
                        ...
                        eval = (isLeaf) ? get_v_bb(&temp, possible_hint)
                                        : evil_expecti_slide(temp,tilebag_status_transfer(tilebag_after_hint, possible_hint ), possible_hint , &next_has_dead_board,bonusTile,totalTile,Alpha,M) ;
            ...
            if(gen_bonus_tile) {
                ...
                for(unsigned i=0;i<emptyslots_length;++i){
                    ...
                    eval = (isLeaf) ? get_v_bb(&temp, 4)
                                    : evil_expecti_slide(temp,tilebag_after_hint, 4 , &next_has_dead_board,bonusTile+1,totalTile,Alpha,M); 
