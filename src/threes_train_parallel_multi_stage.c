...
typedef struct _MSBoard_Slot {
    bitboard msbd;
    indx tile_bag[12];
    unsigned tile_bag_indx;
} MSBoard_Slot;
MSBoard_Slot* MSBoards;
...
int main(int argc, char const *argv[])
{
    ...
    for(i=1;i<argc;++i){
        if (strstr(argv[i],"--weight")) {
            ...
            multistage_path  = strchr(argv[i],'=') + 1;
            if ( multistage_path[0] == '\0') {
                multistage_path  = NULL;
            } 
    ...
    unsigned MSBoards_size;
    
    
    if (multistage_path){
        printf("Load boards file \"%s\"\n",multistage_path);
        FILE* ms_file = fopen(multistage_path,"rb");
        if(!ms_file){
            printf("File Error!\n");
            return 0;
        }
        fread(&MSBoards_size,sizeof(MSBoards_size),1,ms_file);
        printf("Boards Num %d\n",MSBoards_size);
        MSBoards = (MSBoard_Slot *)malloc(sizeof(MSBoard_Slot)*MSBoards_size);
        for (i=0;i<MSBoards_size;++i){
            fread(&(MSBoards[i].msbd),sizeof(bitboard),1,ms_file);
            fread(MSBoards[i].tile_bag,sizeof(indx),12,ms_file);
            fread(&(MSBoards[i].tile_bag_indx),sizeof(unsigned),1,ms_file);
        }

        fclose(ms_file);
        printf("Multi-stage boards loaded!\n");
    } else {
        printf("There is no board for multi-stage training...\n");
        return 0;
    }
    ...
    unsigned MSBoards_index_interval = MSBoards_size / numthreads;
    
    #pragma omp parallel num_threads(numthreads) default(shared) private(i,j)
    {
        unsigned MSBoards_index = MSBoards_index_interval * omp_get_thread_num();
        printf("Thread #%d start  MS Start Index = %d...\n", omp_get_thread_num(),MSBoards_index);
   ...
   unsigned placed_position;
    ...
    for(j=0;j<total_th;++j) {
        ...
        // load MS Board
        bb = MSBoards[MSBoards_index].msbd;
        // copy tile-bag
        for(i=0;i<TILE_BAG_SIZE;++i){
            tile_bag[i] = MSBoards[MSBoards_index].tile_bag[i];
        }
        tile_bag_indx = MSBoards[MSBoards_index].tile_bag_indx;
        ...
        // Next MS Board
        MSBoards_index++;
        if(MSBoards_index>=MSBoards_size){
            MSBoards_index = 0;
        }        
    }
